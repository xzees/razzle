...
export const createServerContext = () => {
  ...
  const resolveData = async (timeout?: number) => {
    const effects = internalContextValue.requests.map((item: any) => item.promise);

    if (timeout) {
      const timeOutPr = wait(timeout);

      await Promise.all(
        internalContextValue.requests.map((effect: any, index: any) => {
