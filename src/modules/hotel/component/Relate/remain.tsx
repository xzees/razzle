import React from 'react';
import { Box } from '@material-ui/core';
import _ from 'lodash';
import styled from 'styled-components';
import ErrorRoundedIcon from '@material-ui/icons/ErrorRounded';
import FontManager from 'modules/hotel/Manager/FontManager';
import i18n from 'utilities/I18n';

interface IMText {
    color?: string;
    lineHeight?: string;
}

export const MText = styled.div`
  
    font-family: ${FontManager.default.secondaryFont};
    position: relative;
    display: inline-block;
    font-size: ${FontManager.default.TEXT_TINY_18} !important;

    a {
        color: #d60c0c !important;
    }
    width: 100%;
    margin-bottom: 10px;
    .tooltiptext {
        background-color: #ffeaea;
        color: #d60c0c !important;
        border: 1px solid #e8e8e8;
        text-align: left;
        padding: 2px 5px;
        border-radius: 5px;
        position: inherit;
        font-size: ${FontManager.default.TEXT_TINY_18} !important;
    }

    .tooltiptextmobile {
        background-color: #ffeaea;
        color: #d60c0c !important;
        border: 1px solid #e8e8e8;
        text-align: left;
        padding: 2px 5px;
        border-radius: 5px;
        position: inherit;
        font-size: ${FontManager.default.SMALL_12};
    }
    
    .tooltiptext::after,.tooltiptextmobile::after {
        content: "";
        position: absolute;
        top: 80%;
        left: 88%;
        margin-left: -4px;
        border-width: 4px;
        border-style: solid;
        border-color: #ffeaea00 #ffeaea #ffeaea #ffeaea00 ;
        -webkit-transform: rotate(45deg);
        -ms-transform: rotate(45deg);
        transform: rotate(45deg);
        box-shadow: 1px 1px 0px 0px #e8e8e8;
    }
`;

const remain = (props: any) => {
    const { hotelRemain } = props;
    if(props.isMobile) {
        return (
            <Box style={{
                textAlign: 'right',
                width: '100%',
                height: '27px',
            }}>
                <MText>
                        <span className="tooltiptextmobile">
                                {i18n.t('hotel.components.relate.remain.lblremain')} {hotelRemain} {i18n.t('hotel.search.room')} &nbsp;
                                <ErrorRoundedIcon 
                                style={{
                                    width: '19px',
                                    verticalAlign: 'middle',
                                    marginRight: '2px',
                                    fontSize: FontManager.default.SMALL_MEDIUM_14,
                                }}
                            />
                        </span>
                </MText>
            </Box>
        )
    }
    return (
        <Box style={{
            textAlign: 'right',
            width: '100%',
            height: '31px',
        }}>
            <MText >
                    <span className="tooltiptext">
                        <a href={'test'}>
                            {i18n.t('hotel.components.relate.remain.lblremain')} {hotelRemain} {i18n.t('hotel.search.room')} &nbsp;
                            <ErrorRoundedIcon 
                            style={{
                                width: '19px',
                                verticalAlign: 'middle',
                                marginRight: '2px',
                            }}
                        />
                        </a>
                    </span>
            </MText>
        </Box>
    )
}

export default remain